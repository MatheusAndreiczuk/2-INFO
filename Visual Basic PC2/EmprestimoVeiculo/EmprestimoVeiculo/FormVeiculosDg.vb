Public Class FormVeiculosDg
    ...
    Private Sub EmprestarToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles EmprestarToolStripMenuItem.Click
        Dim codigo = -1, linha As Integer
        linha = dgFiltro.Rows.GetFirstRow(DataGridViewElementStates.Selected)
        If linha > -1 Then
            If dgFiltro.Rows(linha).Cells(3).Value = 0 Then
                codigo = dgFiltro.Rows(linha).Cells(0).Value
                FormVeiculoEmprestimo.veiculo = bd.localizar(codigo)
                FormVeiculoEmprestimo.novo = False
                FormVeiculoEmprestimo.Show()
            Else
                MsgBox("O veículo já se encontra emprestado!")
            End If
        Else
            MsgBox("Nenhuma linha selecionada")
        End If
    ...
    Private Sub DevolverToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DevolverToolStripMenuItem.Click
        ...
        If linha > -1 Then
            ...
            Else
                MsgBox("O veículo não está emprestado!")
            End If
        Else
            MsgBox("Nenhuma linha selecionada")
        End If
