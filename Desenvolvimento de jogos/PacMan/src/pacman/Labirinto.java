...
public class Labirinto extends JPanel implements ActionListener {
    ...
    public Labirinto(int largura, int altura) {
        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                if (tecla == KeyEvent.VK_ESCAPE) {//Atenção, definição de tecla de saída
                    repaint();
                    t.stop();
                    if (JOptionPane.showConfirmDialog(null, "Deseja realmente sair", "Sair",
                            JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) { //continuação da linha anterior 
                        System.exit(0);
                    } else {
                        t.restart();
                    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (labirinto[pacman.getY() + pacman.getDy()][pacman.getX() + pacman.getDx()] != 'p' && (seq == 0
                || seq == 3)) { //continuação da linha anterior
            pacman.move();
        }
        if (comida == 159) {
            inicializa();
    ...
    private void mexerFantasmas() {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (ghost[i].getStatus() == 0) {
                if (labirinto[ghost[i].getY() + ghost[i].getDy()][ghost[i].getX() + ghost[i].getDx()]
                        != 'p') { //continuação da linha anterior
                    ghost[i].move();
                    if (ghost[i].getDx() != 0) {
            ...
            for (int x = 0; x < 19; x++) {
                switch (labirinto[y][x]) {
                    ...
                    case 'c':
                        ...
                        g.fillArc(x * passoX + (passoX / 3), y * passoY + (passoY / 3), passoX / 2, passoY
                                / 2, 0, 360); //continuação da linha anterior
                        break;
                    case 'o':
